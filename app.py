...
fetch('/catalog').then(r=>r.json()).then(data=>{
  ...
  buildCatChips();
  filterCatalog();
}).catch(e=>console.error('Каталог:', e));
...
function renderResult(d) {
  ...
  document.getElementById('result').innerHTML = `
    ...
    <div class="metrics-grid">
      ...
      <div class="metric-card"><div class="metric-label">Выкуп</div><div class="metric-tooltip">Процент заказов которые покупатель не вернул. Низкий выкуп = высокие затраты на логистику возвратов. Реальные штуки = выручка после выкупа / средняя цена.</div>${(() => { const rwb = d.revenue_with_buyout || d.revenue * (d.buyout_pct||1); const realSales = d.avg_price > 0 ? Math.round(rwb / d.avg_price) : 0; const realStr = realSales > 1000000 ? (realSales/1000000).toFixed(1)+' млн шт/мес' : realSales > 1000 ? Math.round(realSales/1000)+' тыс шт/мес' : realSales+' шт/мес'; const ordStr = d.orders > 1000000 ? (d.orders/1000000).toFixed(1)+' млн заказов' : d.orders > 1000 ? Math.round(d.orders/1000)+' тыс заказов' : d.orders+' заказов'; return '<div class="metric-value">'+((d.buyout_pct*100).toFixed(0))+'% <span style="font-size:13px;font-weight:400;color:#94a3b8;">· '+realStr+'</span></div><div class="metric-sub">'+(d.buyout_pct >= 0.8 ? 'отличный' : d.buyout_pct >= 0.6 ? 'хороший' : 'низкий')+' · из <span style="color:#64748b;">'+ordStr+'</span></div>'; })()}</div>
...
function defaultCompanySettings() {
  return {
    employees: [
      { name: 'Директор / Менеджер (совмещение)', salary: 0, fixed: true },
      { name: 'Бухгалтер (аутсорс)', salary: 0, fixed: true },
...
function resetCompanySettings() {
  if(confirm('Сбросить все настройки компании к нулю?')) {
...
function renderCashflow() {
  ...
  const phaseEnd0   = s.phase0_end   || 1;
  const phaseEnd1   = s.phase1_end   || 3;
  ...
  // Консервативная кривая оборота (база 220k$, масштабируется)
  const sf = capital / 220000;
  const revCurve = [
    0, 4000, 16500, 35000, 47500, 57500, 80000, 102500, 117500, 132500, 143500, 150000
  ].map(v => Math.round(v * sf));

  const months = 24;
  const startDate = s.start_date ? new Date(s.start_date) : new Date();
  let remCapital = workingCapital;
  let remDebt = capital;
  let breakEvenMonth = null, debtFreeMonth = null, peakRevenue = 0;
  let rows = [];
  ...
  for (let i = 0; i < months; i++) {
    ...
    // Оборот по кривой
    let grossRevenue = i < revCurve.length
      ? revCurve[i]
      : Math.round((rows[i-1] ? rows[i-1].grossRevenue : revCurve[revCurve.length-1]) * 1.06);
    ...
    const taxBase       = s.tax_mode === 'usn15'
      ? Math.max(0, netSales - commissionAmt - logisticsAmt - monthlyExp)
      : netSales;
    const taxAmt = Math.round(taxBase * taxRate);

    // Реклама: снижается по мере органики 15%->7% за 14 мес
    const organicGrowth = Math.min(0.55, (i / 14) * 0.55);
    const adsAmt = Math.round(netSales * Math.max(0.07, 0.15 * (1 - organicGrowth)));
    ...
    if (m <= phaseEnd0) {
      ...
      if (mInP===1) { capDraw=monthlyExp+testBudget; capitalNote=`Тест #1: закупка ${testBudget.toLocaleString()}$`; }
      else { capitalNote='Тест: продажи'; }
    } else if (m <= phaseEnd2) {
    ...
    remCapital = Math.max(0, remCapital - capNeeded);

    if (operProfit > 0 && breakEvenMonth === null && phase >= 2) breakEvenMonth = m;
    const returnPct = 0.25; // 25% прибыли на возврат, 75% реинвест
    if (m >= returnStart && operProfit > 0) remDebt = Math.max(0, remDebt - Math.round(operProfit * returnPct));
    if (remDebt === 0 && debtFreeMonth === null) debtFreeMonth = m;
    peakRevenue = Math.max(peakRevenue, grossRevenue);

    let warning = '';
    if (remCapital < monthlyExp * 2 && phase < 4) warning = '⚠️ Мало капитала';
    if (m >= returnStart && operProfit <= 0) warning = '⚠️ Убыток — возврат невозможен';

    rows.push({m, label, phase, phaseName, phaseColor, grossRevenue, netSales,
      commissionAmt, logisticsAmt, taxAmt, adsAmt, operProfit,
      capDraw, capitalNote, remCapital, remDebt, warning});
  }

  const netMarginEff = 1 - wbReturns - wbCommission - wbLogistics - 0.10 - 0.06;
  ...
  if (controlsEl) {
    ...
    controlsEl.innerHTML = pfBlock + `
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px;">
        ${[
          {key:'phase0_end',  label:'🏗️ Подготовка до',val:phaseEnd0,  min:1,max:3},
          {key:'phase1_end',  label:'🧪 Тесты до',     val:phaseEnd1,  min:2,max:6},
  ...
  // Вычисляем метки дат для пояснений
  ...
  const peakMonth = rows.reduce((best, r) => r.grossRevenue > (best ? rows[best-1].grossRevenue : 0) ? r.m : best, 1);
  const returnMonthCount = debtFreeMonth && returnStart ? debtFreeMonth - returnStart + 1 : null;
  ...
  if (chartEl) {
    ...
    const maxD = capital, maxR = Math.max(...rows.map(r=>r.grossRevenue));
    const xS = i=>PL+(i/(months-1))*CW;
    const yD = v=>PT+CH-(v/maxD)*CH;
    const yR = v=>PT+CH-(v/maxR)*CH;
    const dPath = rows.map((r,i)=>`${i===0?'M':'L'}${xS(i).toFixed(1)},${yD(r.remDebt).toFixed(1)}`).join(' ');
    const rPath = rows.map((r,i)=>`${i===0?'M':'L'}${xS(i).toFixed(1)},${yR(r.grossRevenue).toFixed(1)}`).join(' ');
    // Прибыль убрана с графика — только долг и оборот
    const xlbls = rows.filter((_,i)=>i%3===0).map(r=>
      `<text x="${xS(r.m-1).toFixed(1)}" y="${H-8}" fill="#64748b" font-size="10" text-anchor="middle">${r.label}</text>`).join('');
    // Вертикальная линия на первом месяце реальной выплаты долга
    const firstReturnIdx = rows.findIndex(r => r.m >= returnStart && r.operProfit > 0);
    const retL = firstReturnIdx >= 0
      ? `<line x1="${xS(firstReturnIdx)}" y1="${PT}" x2="${xS(firstReturnIdx)}" y2="${PT+CH}" stroke="#34d399" stroke-width="1.5" stroke-dasharray="4,3"/>
         <text x="${xS(firstReturnIdx)+4}" y="${PT+14}" fill="#34d399" font-size="9">начало возврата</text>` : '';
    // breakEven линия
    const beIdx = breakEvenMonth ? breakEvenMonth - 1 : -1;
    const beL = beIdx >= 0
      ? `<line x1="${xS(beIdx)}" y1="${PT}" x2="${xS(beIdx)}" y2="${PT+CH}" stroke="#fbbf24" stroke-width="1" stroke-dasharray="3,3" opacity="0.6"/>
         <text x="${xS(beIdx)+4}" y="${PT+26}" fill="#fbbf24" font-size="9">безубыток</text>` : '';
    chartEl.innerHTML = `
      <div style="color:#94a3b8;font-size:11px;margin-bottom:8px;display:flex;gap:16px;flex-wrap:wrap;">
        <span style="color:#ef4444;">━━ Долг инвестору</span>
        <span style="color:#34d399;">━━ Валовый оборот (продажи покупателям)</span>
      ...
      <svg viewBox="0 0 ${W} ${H}" style="width:100%;background:#0f1117;border-radius:8px;">
        ...
        ${xlbls}
        <text x="${PL-5}" y="${PT+5}"  fill="#64748b" font-size="9" text-anchor="end">${Math.round(maxD/1000)}k$</text>
  ...
  if (tableEl) {
    const tRows = rows.map(r=>{
      ...
      return `<tr style="border-bottom:1px solid #1a2035;${r.warning?'background:#1a1015;':''}">
        ...
        <td style="padding:5px 8px;color:${dc};font-size:11px;font-weight:600;">${r.remDebt.toLocaleString()}$<br><span style="color:#64748b;font-size:10px;">${fmt(r.remDebt)} ${sym}</span></td>
      </tr>`;
    }).join('');
    tableEl.innerHTML = `
      <table style="width:100%;border-collapse:collapse;">
        <thead><tr style="color:#64748b;font-size:10px;border-bottom:1px solid #3b82f633;background:#0f1117;">
          ...
          <th style="padding:6px 8px;text-align:left;">Логистика</th>
...
function companyUpdateEmpName(i, val) {
  const s = getCompanySettings() || defaultCompanySettings();
  s.employees[i].name = val;
  saveCompanySettings(s);
}
function companyUpdateEmpSalary(i, val) {
  const s = getCompanySettings() || defaultCompanySettings();
  s.employees[i].salary = Number(val);
  saveCompanySettings(s);
  renderCompany();
}
function companyUpdateWarehouse(field, val) {
  const s = getCompanySettings() || defaultCompanySettings();
  if(field==='m2') s.warehouse_m2 = Number(val);
  else s.warehouse_rate = Number(val);
  saveCompanySettings(s);
  renderCompany();
}
function companyUpdateTax(mode) {
  const s = getCompanySettings() || defaultCompanySettings();
  s.tax_mode = mode;
  saveCompanySettings(s);
}
...
function companyUpdateCapital(val) {
  const s = getCompanySettings() || defaultCompanySettings();
  s.start_capital = Number(val);
  saveCompanySettings(s);
  renderCompany();
}
function companyAddEmployee() {
  const s = getCompanySettings() || defaultCompanySettings();
  s.employees.push({ name: '', salary: 0, fixed: false, placeholder: 'Должность сотрудника' });
  saveCompanySettings(s);
  renderCompany();
}
function companyRemoveEmployee(i) {
  const s = getCompanySettings() || defaultCompanySettings();
  if(s.employees[i] && !s.employees[i].fixed) {
  ...
  if (filtered.length === 0) {
    ...
    filtered.forEach(function(item, localIdx) {
      ...
      if (activeTab === 'draft') {
        ...
        html += '<div style="text-align:center;"><div style="font-size:9px;color:#555;margin-bottom:3px;">ДОЗАКАЗ</div><button onclick="reorderItem(' + globalIdx + ')" style="background:#3b82f622;border:1px solid #3b82f644;border-radius:5px;color:#93c5fd;padding:4px 8px;cursor:pointer;font-size:11px;">&#128257; Заказать снова</button></div>';
        html += '</div>';
...
function renderDocsResult(data) {
  ...
  if (r.customs_docs && r.customs_docs.length > 0) {
    ...
    r.customs_docs.forEach(function(doc) {
      var docName = typeof doc === 'object' ? (doc.name || '') : doc;
      var docDesc = typeof doc === 'object' ? (doc.description || '') : '';
      html += '<div style="display:flex;gap:8px;margin-bottom:8px;align-items:flex-start;">' +
        '<div style="color:#38bdf8;font-size:14px;">&#128230;</div>' +
        '<div><div style="font-size:12px;color:#aaa;line-height:1.5;">' + docName + '</div>' +
        (docDesc ? '<div style="font-size:11px;color:#555;">' + docDesc + '</div>' : '') +
        '</div></div>';
    });
    html += '</div>';
  }
  ...
  if (r.risks && r.risks.length > 0) {
    ...
    r.risks.forEach(function(risk) {
      var riskText = typeof risk === 'object' ? (risk.risk || '') : risk;
      var riskSolution = typeof risk === 'object' ? (risk.solution || '') : '';
      html += '<div style="display:flex;gap:8px;margin-bottom:8px;align-items:flex-start;">' +
        '<div style="color:#fbbf24;font-size:14px;">&#9888;</div>' +
        '<div><div style="font-size:12px;color:#aaa;line-height:1.5;">' + riskText + '</div>' +
        (riskSolution ? '<div style="font-size:11px;color:#4ade80;">&#10003; ' + riskSolution + '</div>' : '') +
        '</div></div>';
    });
    html += '</div>';
  }
...
Только JSON без markdown."""
            ...
            try:
                ...
                prompt = f"""Пользователь ищет нишу на Wildberries по запросу: "{query}"
...
Правила:
- Ищи по смысловому значению товара
- Тестораскатка/лапшерезка = кухонный инструмент → ищи в кухонных нишах
- Смеситель кухонный = сантехника → ищи "Смесители"
- Если запрос многословный — определи основной товар и найди его нишу
- НЕ интерпретируй слова буквально если есть очевидный смысл
- Возвращай ТОЛЬКО одну самую релевантную нишу из списка
            ...
            try:
                ...
                if not row:
                    ...
                    result = {
                        ...
                        'revenue_with_buyout': float(revenue_with_buyout or 0),
                        'orders': int(orders or 0),
...
{{
  ...
  "detail": "3-4 предложения: почему этот сценарий лучше, конкретные цифры, что нужно сделать для старта, риски"
            ...
            try:
                import anthropic, urllib.request, base64, re
                length = int(self.headers.get('Content-Length', 0))
                body = json.loads(self.rfile.read(length))
                ...
                # Пытаемся загрузить фото с WB если нет пользовательского
                image_content = None
                photo_source = 'text'

                if photo_base64:
                    # Пользователь загрузил фото
                    image_content = {'type': 'base64', 'media_type': photo_media_type, 'data': photo_base64}
                    photo_source = 'user_upload'
                elif item_id:
                    # Пробуем загрузить фото с WB
                    try:
                        vol = str(int(item_id) // 100000)
                        part = str(int(item_id) // 1000)
                        photo_url = f'https://basket-{str(int(vol)//100+1).zfill(2)}.wbbasket.ru/vol{vol}/part{part}/{item_id}/images/big/1.webp'
                        req = urllib.request.Request(photo_url, headers={'User-Agent': 'Mozilla/5.0'})
                        with urllib.request.urlopen(req, timeout=5) as resp:
                            img_data = resp.read()
                            img_b64 = base64.b64encode(img_data).decode('utf-8')
                            image_content = {'type': 'base64', 'media_type': 'image/webp', 'data': img_b64}
                            photo_source = 'wb_auto'
                    except Exception as img_err:
                        photo_source = 'text_fallback'

                # Формируем промпт
                json_schema = '{"item_id": "строка", "photo_source": "строка", "characteristics": "детальное описание", "price_1688_usd": число, "price_alibaba_usd": число, "price_mic_usd": число, "moq": число, "search_links": [{"platform": "1688", "url": "https://s.1688.com/selloffer/offer_search.htm?keywords=ЗАПРОС_CN", "icon": "строка", "query_hint": "запрос на китайском"}, {"platform": "Alibaba", "url": "https://www.alibaba.com/trade/search?SearchText=ЗАПРОС_EN", "icon": "строка", "query_hint": "запрос на английском"}, {"platform": "Made-in-China", "url": "https://www.made-in-china.com/multi-search/ЗАПРОС_EN/F1/0.html", "icon": "строка", "query_hint": "запрос на английском"}, {"platform": "AliExpress", "url": "https://www.aliexpress.com/wholesale?SearchText=ЗАПРОС_EN", "icon": "строка", "query_hint": "для проверки розничной цены"}, {"platform": "Pinduoduo", "url": "https://mobile.yangkeduo.com/search_result.html?search_key=ЗАПРОС_CN", "icon": "строка", "query_hint": "низкие цены групповые закупки"}, {"platform": "Bao66", "url": "https://www.bao66.cn/search/ЗАПРОС_CN.html", "icon": "строка", "query_hint": "если товар - сумки рюкзаки чемоданы"}, {"platform": "Qipeiren", "url": "http://mall.qipeiren.com/Search/Index?keyword=ЗАПРОС_CN", "icon": "строка", "query_hint": "если товар - автозапчасти"}], "recommendation": "3-4 предложения"}'
                ...
                result['photo_source'] = photo_source
            ...
            try:
                ...
                json_tmpl = '{"price_taobao_usd": 0, "price_alibaba_usd": 0, "moq": 0, "summary": "текст", "search_links": [{"platform": "1688", "url": "https://s.1688.com/selloffer/offer_search.htm?keywords=QUERY_CN", "icon": "строка", "description": "самые низкие оптовые цены"}, {"platform": "Alibaba", "url": "https://www.alibaba.com/trade/search?SearchText=QUERY_EN", "icon": "строка", "description": "оптовые поставщики"}, {"platform": "Made-in-China", "url": "https://www.made-in-china.com/multi-search/QUERY_EN/F1/0.html", "icon": "строка", "description": "проверенные производители"}, {"platform": "Pinduoduo", "url": "https://mobile.yangkeduo.com/search_result.html?search_key=QUERY_CN", "icon": "строка", "description": "низкие цены групповые закупки"}, {"platform": "AliExpress", "url": "https://www.aliexpress.com/wholesale?SearchText=QUERY_EN", "icon": "строка", "description": "розничные цены для оценки"}, {"platform": "Taobao", "url": "https://s.taobao.com/search?q=QUERY_CN", "icon": "строка", "description": "розница и мелкий опт"}, {"platform": "Bao66 (сумки)", "url": "https://www.bao66.cn/search/QUERY_CN.html", "icon": "строка", "description": "сумки рюкзаки чемоданы Байгоу"}, {"platform": "Qipeiren (авто)", "url": "http://mall.qipeiren.com/Search/Index?keyword=QUERY_CN", "icon": "строка", "description": "автозапчасти и комплектующие"}], "real_margin_pct": 0, "roi_pct": 0, "profit_per_unit_rub": 0}'
                p.append(json_tmpl)
...
Все суммы в рублях. Будь конкретным — никаких размытых диапазонов."""
            ...
            try:
                ...
                agent_name = self.path[len('/agent/'):].split('?')[0].strip('/')
                length = int(self.headers.get('Content-Length', 0))
                body_bytes = self.rfile.read(length)
                payload = json.loads(body_bytes)
                niche = dict(payload.get('niche') or {})
                print(f'[AGENT] {agent_name} для {niche.get("name","")}')
                import pdf_auto
                result = pdf_auto.run_agent(agent_name, niche)
...
def handle_supplier_stream(handler, body):
    ...
    p.append('{"price_taobao_usd": 0, "price_alibaba_usd": 0, "moq": 0, "summary": "\u0442\u0435\u043a\u0441\u0442", "search_links": [{"platform": "1688", "url": "https://s.1688.com/selloffer/offer_search.htm?keywords=QUERY_CN", "icon": "\U0001f1e8\U0001f1f3", "description": "\u0434\u0435\u0448\u0435\u0432\u043b\u0435"}, {"platform": "Alibaba", "url": "https://www.alibaba.com/trade/search?SearchText=QUERY_EN", "icon": "\U0001f310", "description": "\u043e\u043f\u0442"}, {"platform": "Pinduoduo", "url": "https://mobile.yangkeduo.com/search_result.html?search_key=QUERY_CN", "icon": "\U0001f6d2", "description": "\u043d\u0438\u0437\u043a\u0438\u0435 \u0446\u0435\u043d\u044b"}, {"platform": "AliExpress", "url": "https://www.aliexpress.com/wholesale?SearchText=QUERY_EN", "icon": "\U0001f6cd", "description": "\u0440\u043e\u0437\u043d\u0438\u0446\u0430"}, {"platform": "Bao66", "url": "https://www.bao66.cn/search/QUERY_CN.html", "icon": "\U0001f45c", "description": "\u0441\u0443\u043c\u043a\u0438/\u0447\u0435\u043c\u043e\u0434\u0430\u043d\u044b"}, {"platform": "Qipeiren", "url": "http://mall.qipeiren.com/Search/Index?keyword=QUERY_CN", "icon": "\U0001f697", "description": "\u0430\u0432\u0442\u043e\u0437\u0430\u043f\u0447\u0430\u0441\u0442\u0438"}], "real_margin_pct": 0, "roi_pct": 0, "profit_per_unit_rub": 0}')
    prompt = '\n'.join(p)
...
def handle_docs_stream(handler, body):
    ...
    prompt = (
        f"Ты эксперт по сертификации и документам для торговли на Wildberries.\n"
        f"Компания зарегистрирована в Республике Беларусь (Минск), закупает товары в Китае.\n\n"
        f"НИША: {display_name}\n"
        f"Средняя цена: {avg_price} руб\n\n"
        "Дай детальный анализ по 5 блокам:\n\n"
        "1. ДОКУМЕНТЫ ДЛЯ WB — какие документы требует Wildberries для этой категории\n"
        "2. СЕРТИФИКАЦИЯ В ЕАЭС — нужна ли сертификация, декларация или отказное письмо\n"
        "3. ГДЕ ПОЛУЧИТЬ ДОКУМЕНТЫ:\n"
        "   - В РБ: БелГИСС (belst.by), БГЦА (bgca.by), аккредитованные лаборатории\n"
        "   - В РФ: Роспотребнадзор, ФБУ Ростест-Москва (rostest.ru), ВНИИС\n"
        "   - Сравни где выгоднее оформить — РБ или РФ (цена, сроки)\n"
        "4. ЧТО ЗАПРОСИТЬ У КИТАЙСКОГО ПОСТАВЩИКА:\n"
        "   - Certificate of Analysis (CoA)\n"
        "   - ISO сертификаты производства\n"
        "   - CE маркировка (если применимо)\n"
        "   - Test reports от аккредитованных лабораторий\n"
        "   - MSDS (для химии/косметики)\n"
        "   - Укажи какие из них принимаются в ЕАЭС без переоформления\n"
        "5. ТАМОЖНЯ РБ — документы для ввоза из Китая, коды ТН ВЭД, особенности\n\n"
        "ВАЖНО: информация справочная, требует проверки в актуальных источниках.\n\n"
        "Верни ONLY валидный JSON без markdown:\n"
        "{{\n"
        '  "complexity": "low|medium|high",\n'
        '  "complexity_reason": "почему такая сложность",\n'
        '  "summary": "3-4 предложения — главное что нужно знать",\n'
        '  "wb_docs": [{{"name": "название", "required": true, "description": "описание", "cost": "цена", "duration": "срок", "where_rb": "организация в РБ", "where_rf": "организация в РФ", "better_in": "РБ или РФ и почему"}}],\n'
        '  "supplier_docs": [{{"name": "документ", "description": "что даёт", "accepted_in_eaes": true, "how_to_request": "как запросить у поставщика"}}],\n'
        '  "customs_docs": [{{"name": "документ", "description": "для чего"}}],\n'
        '  "risks": [{{"risk": "риск", "solution": "решение"}}],\n'
        '  "total_cost": "стоимость в руб",\n'
...
def handle_warehouse_stream(handler, body):
    ...
    prompt = (
        ...
        "НЕ рекомендуй склады Электросталь, Краснодар, Невинномысск, Шушары, Новосаратовка, Котовск, "
        "Симферополь, Волгоград — "
