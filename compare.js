async function compareAllWL() {
  var allList = getWatchlist();
  if (allList.length === 0) { alert("Нет ниш в работе!"); return; }

  // Если есть отмеченные галочками - берём только их, иначе все
  var checked = window._wlChecked || [];
  var list = checked.length > 0 ? checked.map(function(i){ return allList[i]; }) : allList;

  if (list.length === 0) { alert("Нет ниш для сравнения!"); return; }
  ...
  var label = checked.length > 0 ? checked.length + " выбранных ниш" : list.length + " ниш в работе";
  ...
  html += "<div style='font-size:11px;color:#555;'>" + label + "</div>";
  ...
  // Сохраняем список для AI
  window._compareList = list;
...
async function runCompareAI() {
  var list = window._compareList || getWatchlist();
