...
PLATFORMS: dict[str, list[dict]] = {
    "china": [
        ...
        {
            # Перемещено на 2-е место 05.08.2026: раньше стояло 5-м и никогда не
            # попадало в PDF из-за среза топ-3 платформ (см. Правку 4) — критичный
            # для закупки в Китае инструмент был фактически скрыт от пользователя.
            "name": "Агенты в Китае (Yiwi, Guangzhou)",
            "url_tpl": None,
            "note": "Проверка качества (QC), упаковка под бренд, консолидация грузов, карго-доставка в РФ/РБ. Комиссия 5–10%.",
            "moq": "от 1 упаковки",
        },
        {
            "name": "Alibaba.com",
    ...
    "russia": [
        {
            # 3А, 05.08.2026: было "puls-cen.ru" — опечатка в домене (лишний
            # дефис), DNS не резолвился вообще. Реальный домен без дефиса.
            "name": "Пульс цен (pulscen.ru)",
            "url_tpl": "https://pulscen.ru/search/?q={ru}",
            "note": "Крупнейший агрегатор российских оптовых поставщиков. Цены обновляются ежедневно.",
            "moq": "от 1 упаковки",
        },
        {
            # Правка 2, 05.08.2026 — СРОЧНО: было "Tiu.ru" — домен сменил
            # владельца, сейчас это сайт про ставки на спорт (репутационный
            # риск). Заменено на ruopt.com — маркетплейс именно для продавцов
            # WB/Ozon, специализация точно по аудитории продукта. Поставлено
            # на 2-е место (было бы 4-м — не попадало бы в топ-3 [:3] и не
            # показывалось бы вообще, хотя это самая релевантная площадка
            # из всех российских именно для WB-продавца).
            "name": "РУОПТ (ruopt.com)",
            "url_tpl": "https://ruopt.com/search/?q={ru}",
            "note": "Маркетплейс для продавцов WB и Ozon. Каталог товаров + фулфилмент + импорт карточек в кабинет.",
            "moq": "от 1 единицы",
        },
        {
            # 3Б, 05.08.2026: было "Optom.ru" — SSL-ошибки при проверке,
            # независимых подтверждений актуальности не нашлось. Заменено.
            "name": "Productcenter.ru",
            "url_tpl": "https://productcenter.ru/products/?search={ru}",
            "note": "22 000+ российских производителей, региональные фильтры, запрос цен оптом.",
            "moq": "от 1 партии",
        },
        {
            # 3В, 05.08.2026: было "Produced.ru" — сайт не отвечал (connection
            # refused), заменено.
            "name": "ОптЛист (optlist.ru)",
            "url_tpl": "https://optlist.ru/search/?q={ru}",
            "note": "B2B база поставщиков РФ, работает 15+ лет.",
            "moq": "от 1 коробки",
        },
        {
...
CATEGORY_KEYWORDS: dict[str, list[str]] = {
    ...
    "appliances": [
        "вытяжка", "вытяжки", "плита", "духовой шкаф", "посудомоечная",
        "холодильник", "стиральная", "кондиционер", "вентилятор",
        "обогреватель", "тепловентилятор", "водонагреватель", "котёл",
        "варочная панель", "микроволновка", "блендер", "миксер",
        "кофемашина", "пылесос", "утюг", "фен",
    ],
    "tools_hardware": [
        "инструмент", "дрель", "шуруповёрт", "перфоратор", "лобзик",
        "болгарка", "шлифовальная", "молоток", "отвёртка", "гаечный ключ",
        "уровень строительный", "рулетка", "степлер", "заклёпочник",
        "фурнитура", "петли", "ручки мебельные", "замки", "крепёж",
        "дюбель", "саморез", "анкер", "болт", "гайка",
    ],
    "garden_outdoor": [
        "газонокосилка", "триммер", "мотоблок", "культиватор",
        "мангал", "барбекю", "гриль", "садовый", "огород",
        "теплица", "горшок цветочный", "кашпо", "лейка",
        "шланг поливочный", "насос садовый", "грабли", "лопата",
        "тачка садовая", "качели", "гамак", "зонт садовый",
    ],
    "furniture_kitchen": [
        "тумба", "комод", "шкаф", "стеллаж", "полка",
        "стол кухонный", "стул", "табурет", "диван", "кресло",
        "кровать", "матрас", "подушка декоративная", "пуф",
        "вешалка", "зеркало", "картина", "рамка для фото",
    ],
    "baby_kids": [
        "коляска", "автокресло", "кроватка детская", "манеж",
        "ходунки", "прыгунки", "качели детские", "пустышка",
        "соска", "бутылочка", "подгузник", "слюнявчик",
        "игрушка", "конструктор", "кубики", "пазл", "кукла",
    ],
    "sports_fitness": [
        "гантели", "гиря", "штанга", "турник", "брусья",
        "коврик для йоги", "скакалка", "эспандер", "тренажёр",
        "велотренажёр", "беговая дорожка", "боксёрская груша",
        "бассейн надувной", "самокат", "велосипед", "ролики",
    ],
}
...
SOURCING_MATRIX: dict[str, dict[str, dict[str, float]]] = {
    ...
    # Дефолт для неизвестных категорий — china-first
    "_default": {
        "economy": {"china": 8.0, "russia": 5.0, "turkey": 3.0, "india": 2.0, "europe": 2.0},
        "mid":     {"china": 8.0, "russia": 5.0, "turkey": 3.0, "india": 2.0, "europe": 2.0},
        "premium": {"china": 8.0, "russia": 5.0, "turkey": 3.0, "india": 2.0, "europe": 2.0},
    },
}
